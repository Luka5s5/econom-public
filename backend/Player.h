...
void print(std::string name, std::vector<double> v) {
    std::cout << name << ": ";
    for (auto t: v) 
        std::cout << t << " ";
    std::cout << std::endl;
...
struct Player {
    ...
    int strategy;
    void mine_resources() {
        resources[0] += building_level[0] * 4;
        resources[1] += building_level[1] * 8;
        resources[2] += building_level[2] * 4;
        resources[3] += building_level[3] * 8;
        resources[4] += building_level[4] * 6;
        resources[5] += building_level[5] * 4;
        resources[6] += building_level[6] * building_level[6];
        for (int i = 0; i <= 2; i++) {
            resources[i] += building_level[7] * (building_level[i] - 1);
        }
        for (int i = 3; i <= 5; i++) {
            resources[i] += building_level[8] * (building_level[i] - 1);
        }
        for (int i = 0; i < 7; i++) {
            resources[i] += building_level[9] * 2;
        }
        resources[6] += building_level[8] * (building_level[i] + 1); 
    }
