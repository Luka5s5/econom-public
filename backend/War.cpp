...
{
	for(auto aid:a_side_ids){
		vector<int> temp=Game::current.get_player_army(aid);
	...
	for(auto did:d_side_ids){
		vector<int> temp=Game::current.get_player_army(did);
	...
	for (auto aid : a_side_ids)
	{
		...
		for (int tp = 0; tp < 3; tp++)
			for (int i = 0; i < ith_army[tp].size(); i++)
	...
	for (auto did : d_side_ids)
	{
		...
		for (int tp = 0; tp < 3; tp++)
			for (int i = 0; i < ith_army[tp].size(); i++)
								d_army.push_back(make_pair(tp,did);
	}
}
...
bool War::progress_war()
{
	...
	while (a_army.size() != 0 and d_army.size() != 0 and (a_army.size() / total_att >= survivors_pc) and (d_army.size() / total_def >= survivors_pc))
	{
		...
		if (roll <= a_wins)
		{
			...
			d_army.remove(std::find(d_army.begin(), d_army.end(), std::make_pair(-1, -1)));
			Game::current.kill_troop(d_troop_id,d_troop_type);
		}
		else{
			a_army[a_troop_n] = std::make_pair(-1,-1);
			a_army.renove(std::find(a_army.begin(),a_army.end(), std::make_pair(-1,-1)));
